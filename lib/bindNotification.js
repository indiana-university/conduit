...
export function bindNotification(source, notification, fnName) {
  if(source instanceof Observable)
    return createBindNotification(source, notification, fnName);
...
function keyFromStreamKey(key) {
  ...
  // There's probably a more elegant regex way to do this.
  return key.slice(-1) === '$' ? key.substr(0, key.length-1) : key;
}
